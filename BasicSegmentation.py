...
class BasicSegmentation:
    def __init__(self, input_data, data_folder=None):
        ...
        if isinstance(input_data, str):
            ...
            raise TypeError("`input_data` must be a filepath or a DataFrame row with `filename`.")
    ...
    def visualize_morphology_slice(self, slice_idx=None, se_size=3):
        if self.binarized is None:
            raise ValueError("Run `binarize()` before calling this method.")

        # Choose slice
        if slice_idx is None:
            slice_idx = self.binarized.shape[2] // 2  # middle slice

        binary_slice = self.binarized[:, :, slice_idx]
        selem = Morpho.square(se_size)

        # Apply 2D erosion and dilation
        eroded = Morpho.binary_erosion(binary_slice, selem)
        dilated = Morpho.binary_dilation(eroded, selem)

        # Plot
        fig, axs = plt.subplots(1, 3, figsize=(12, 4))
        axs[0].imshow(binary_slice, cmap='gray')
        axs[0].set_title("(a) Raw Thresholded Mask")

        axs[1].imshow(eroded, cmap='gray')
        axs[1].set_title("(b) After Erosion")

        axs[2].imshow(dilated, cmap='gray')
        axs[2].set_title("(c) After Dilation")

        for ax in axs:
            ax.axis('off')

        plt.suptitle(f"Morphological Operations on Slice {slice_idx}", fontsize=14)
        plt.tight_layout()
        plt.show()
